...
export class HelpCommand extends Command {
			...
			(builder) => {
				builder
					...
					.addStringOption((option) => {
						option.setName('search').setDescription('Get information about a command.').setRequired(true);
	...
	async search(interaction, args) {
		...
		const successEmbed = new EmbedBuilder()
			...
			.setDescription(
				`${await languagePassthrough(interaction, 'help:CommandSuccessName', { '%COMMAND_NAME%': command.name })}\n${await languagePassthrough(interaction, 'help:CommandSuccessDescription', {
